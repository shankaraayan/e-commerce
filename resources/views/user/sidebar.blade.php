      ...
      <div id="collapseOne" class="collapse show user_dashboard" aria-labelledby="headingOne" data-parent="#accordion">
        <div class="card-body p-1 pt-2">
            <div class="nav flex-column nav-pills w-100" id="v-pills-tab" role="tablist" aria-orientation="vertical">
                <a href="{{route('user.dashboard')}}" class="d-flex nav-link rounded-0 align-items-center justify-content-between ps-block__title text-left fs-3 py-3 {{(Route::currentRouteName()=='user.dashboard')? 'active':''}}">
                     Dashboard <i class="icon-grid fs-3 fw-700"></i>
                </a>
                <a href="{{route('user.orders')}}" class="d-flex nav-link rounded-0 align-items-center justify-content-between ps-block__title text-left fs-3 py-3 {{(Route::currentRouteName()=='user.orders')? 'active':''}}" id="orders"  type="button" role="tab" aria-controls="v-pills-orders" aria-selected="false">
                    Orders <i class="icon-outbox fs-3 fw-700"></i>
                </a>
                <a href="{{route('user.address')}}" class="d-flex nav-link rounded-0 align-items-center justify-content-between ps-block__title text-left fs-3 py-3 {{(Route::currentRouteName()=='user.address')? 'active':''}}" >
